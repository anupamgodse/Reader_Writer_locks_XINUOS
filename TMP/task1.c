#include <conf.h>
#include <kernel.h>
#include <proc.h>
#include <lock.h>
#include <stdio.h>
#include <lock.h>

#define DEFAULT_LOCK_PRIO 20

#define assert(x,error) if(!(x)){ \
            kprintf(error);\
            return;\
            }
int mystrncmp(char* des,char* target,int n){
    int i;
    for (i=0;i<n;i++){
        if (target[i] == '.') continue;
        if (des[i] != target[i]) return 1;
    }
    return 0;
}

/*--------------------------------Test 1--------------------------------*/
 
void reader1 (char *msg, int lck)
{
	lock (lck, READ, DEFAULT_LOCK_PRIO);
	kprintf ("  %s: acquired read lock, sleep 2s\n", msg);
	sleep (2);
	kprintf("%s upgrading to write\n", msg);
	lock (lck, WRITE, DEFAULT_LOCK_PRIO+10);
	kprintf ("  %s: upgrage to write lock success, sleep 2s\n", msg);
	sleep (2);
	kprintf ("  %s: to release lock\n", msg);
	releaseall (1, lck);
}

void test1 ()
{
	int	lck;
	int	pid1;
	int	pid2;

	kprintf("\nTest 1: readers can share the rwlock\n");
	lck  = lcreate ();
	assert (lck != SYSERR, "Test 1 failed");

	pid1 = create(reader1, 2000, 20, "reader a", 2, "reader a", lck);
	pid2 = create(reader1, 2000, 20, "reader b", 2, "reader b", lck);


	kprintf("resuming reader a\n");
	resume(pid1);
	sleep(3);
	kprintf("resuming reader b\n");
	resume(pid2);
	
	
	sleep (10);
	kprintf("deleting %d\n", lck);
	ldelete (lck);
	kprintf ("Test 1 ok\n");
}

/*----------------------------------Test 2---------------------------*/
char output2[10];
int count2;
void reader2 (char msg, int lck, int lprio)
{
        int     ret;

        kprintf ("  %c: to acquire lock\n", msg);
        lock (lck, READ, lprio);
        output2[count2++]=msg;
        kprintf ("  %c: acquired lock, sleep 3s\n", msg);
        sleep (3);
        output2[count2++]=msg;
        kprintf ("  %c: to release lock\n", msg);
	releaseall (1, lck);
}

void writer2 (char msg, int lck, int lprio)
{
	kprintf ("  %c: to acquire lock\n", msg);
        lock (lck, WRITE, lprio);
        output2[count2++]=msg;
        kprintf ("  %c: acquired lock, sleep 3s\n", msg);
        sleep (1);
        output2[count2++]=msg;
        kprintf ("  %c: to release lock\n", msg);
        releaseall (1, lck);
}

void test2 ()
{
        count2 = 0;
        int     lck;
        int     rd1, rd2, rd3, rd4;
        int     wr1;

        kprintf("\nTest 2: wait on locks with priority. Expected order of"
		" lock acquisition is: reader A, reader B, reader D, writer C & reader E\n");
        lck  = lcreate ();
        assert (lck != SYSERR, "Test 2 failed");

	rd1 = create(reader2, 2000, 20, "reader2", 3, 'A', lck, 20);
	rd2 = create(reader2, 2000, 20, "reader2", 3, 'B', lck, 30);
	rd3 = create(reader2, 2000, 20, "reader2", 3, 'D', lck, 25);
	rd4 = create(reader2, 2000, 20, "reader2", 3, 'E', lck, 20);
        wr1 = create(writer2, 2000, 20, "writer2", 3, 'C', lck, 25);
	
        kprintf("-start reader A, then sleep 1s. lock granted to reader A\n");
        resume(rd1);
        sleep (1);

        kprintf("-start writer C, then sleep 1s. writer waits for the lock\n");
        resume(wr1);
        sleep (1);


        kprintf("-start reader B, D, E. reader B is granted lock.\n");
        resume (rd2);
	resume (rd3);
	resume (rd4);


        sleep (15);
        kprintf("output=%s\n", output2);
    // ABD(ABD in arbitrary orders)CCEE
        assert(mystrncmp(output2,"ABABDDCCEE",10)==0,"Test 2 FAILED\n");
        kprintf ("Test 2 OK\n");
}

/*----------------------------------Test 3---------------------------*/
void reader3 (char *msg, int lck)
{
        int     ret;

        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, READ, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock\n", msg);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck);
}

void writer3 (char *msg, int lck)
{
        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, WRITE, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock, sleep 10s\n", msg);
        sleep (10);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck);
}

void test3 ()
{
        int     lck;
        int     rd1, rd2;
        int     wr1;

        kprintf("\nTest 3: test the basic priority inheritence\n");
        lck  = lcreate ();
        assert (lck != SYSERR, "Test 3 failed");

        rd1 = create(reader3, 2000, 25, "reader3", 2, "reader A", lck);
        rd2 = create(reader3, 2000, 30, "reader3", 2, "reader B", lck);
        wr1 = create(writer3, 2000, 20, "writer3", 2, "writer", lck);

        kprintf("-start writer, then sleep 1s. lock granted to write (prio 20)\n");
        resume(wr1);
        sleep (1);

        kprintf("-start reader A, then sleep 1s. reader A(prio 25) blocked on the lock\n");
        resume(rd1);
        sleep (1);
	assert (getprio(wr1) == 25, "Test 3 failed");

        kprintf("-start reader B, then sleep 1s. reader B(prio 30) blocked on the lock\n");
        resume (rd2);
	sleep (1);
	assert (getprio(wr1) == 30, "Test 3 failed");
	
	kprintf("-kill reader B, then sleep 1s\n");
	kill (rd2);
	sleep (1);
	assert (getprio(wr1) == 25, "Test 3 failed");

	kprintf("-kill reader A, then sleep 1s\n");
	kill (rd1);
	sleep(1);
	assert(getprio(wr1) == 20, "Test 3 failed");

        sleep (8);
        kprintf ("Test 3 OK\n");
}

/*----------------------------------Test 4---------------------------*/
void lowests (char *msg, int sem)
{
	int i, j;
        kprintf ("  %s: to acquire sem\n", msg);
	wait(sem);
        kprintf ("  %s: acquired sem, perform computation\n", msg);
	for(i=0;i<10000000;i++) {
		if((i % 1000000) == 0)
			kprintf("lowests working\n");
		for(j=0;j<100;j++);
	}
        kprintf ("  %s: to release sem\n", msg);
	signal(sem);
}
void highests (char *msg, int sem)
{
	int i, j;
        kprintf ("  %s: to acquire sem\n", msg);
	wait(sem);
        kprintf ("  %s: acquired sem\n", msg);
	//sleep(1);
        kprintf ("  %s: to release sem\n", msg);
	signal(sem);
}
void mediums (char *msg)
{
        kprintf ("  %s RUNNING (should run last, but here as we are using XINU" 
		"semaphores it runs before highests hence indicating priority inversion)\n", msg);
	sleep(1);
}

void test4 ()
{
        int     sem;
        int     l=10, m=15, h=18, lowestpid, mediumpid, highestpid;

        kprintf("\nTest 4: test the priority inversion problem using XINU semaphores\n");
        sem  = screate (1);

        lowestpid = create(lowests, 2000, l, "lowests", 2, "lowests", sem);
        mediumpid = create(mediums, 2000, m, "mediums", 2, "mediums");
        highestpid = create(highests, 2000, h, "highests", 2, "highests", sem);

        kprintf("-start lowest prio process(prio = 10), then sleep 1s\n");
        resume(lowestpid);
        sleep (1);
        
	kprintf("-start highest prio process(prio = 18), then sleep 1s\n");
        resume(highestpid);
	//sleep(1);

	kprintf("lowest process has prio = %d\n", getprio(lowestpid));
        kprintf("-start medium prio process(prio = 15), then sleep 1s\n");
        resume(mediumpid);
        sleep (1);


        sleep (30);
        kprintf ("Test 4 OK\n");
}

/*----------------------------------Test 5---------------------------*/
void lowest (char *msg, int lck)
{
	int i, j;
        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, WRITE, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock, perform computation\n", msg);
	for(i=0;i<10000000;i++) {
		if((i % 1000000) == 0)
			kprintf("lowest working\n");
		for(j=0;j<100;j++);
	}
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck);
}
void highest (char *msg, int lck)
{
	int i, j;
        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, WRITE, DEFAULT_LOCK_PRIO);
        kprintf ("  %s: acquired lock\n", msg);
	//sleep(1);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck);
}
void medium (char *msg)
{
        kprintf ("  %s RUNNING (should run last, as we have overcome priority inversion"
		" using priority inheritance mechanism %s runs last as it should because"
		" there was highest priority process waiting\n", msg, msg);
	sleep(1);
}

void test5 ()
{
        int     lck;
        int     l=10, m=15, h=18, lowestpid, mediumpid, highestpid;

        kprintf("\nTest 5: test the priority inversion problem using read/write locks\n");
        lck  = lcreate ();
        assert (lck != SYSERR, "Test 5 failed");

        lowestpid = create(lowest, 2000, l, "lowest", 2, "lowest", lck);
        mediumpid = create(medium, 2000, m, "medium", 2, "medium");
        highestpid = create(highest, 2000, h, "highest", 2, "highest", lck);

        kprintf("-start lowest prio process(prio = 10), then sleep 1s\n");
        resume(lowestpid);
        sleep (1);
        
	kprintf("-start highest prio process(prio = 18), then sleep 1s\n");
        resume(highestpid);
	//sleep(1);

	kprintf("lowest process has prio = %d\n", getprio(lowestpid));
        kprintf("-start medium prio process(prio = 15), then sleep 1s\n");
        resume(mediumpid);
        sleep (1);


        sleep (30);
        kprintf ("Test 5 OK\n");
}
int main( )
{
        /* These test cases are only used for test purpose.
         * The provided results do not guarantee your correctness.
         * You need to read the PA2 instruction carefully.
         */
	//test1();
	//test2();
	//test3();
	test4(); //checking priority inversion for semaphohres
	test5(); //checking priority inversion for reader/writer locks implementation

        /* The hook to shutdown QEMU for process-like execution of XINU.
         * This API call exists the QEMU process.
         */
        shutdown();
}



