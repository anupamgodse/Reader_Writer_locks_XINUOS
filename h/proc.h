...
struct	pentry	{
	...
	int 	plockret;
	int	plock;			/*lock if proc is waitiing*/
	int	plwtype;		/*waiting for lock type if waiting*/ 
	int 	plprio;			/*lock priority if waiting*/
	unsigned long lclkstart;	/*waiting for lock since*/
	
	unsigned int llocks;		/*bitmap to indicate locks(0-31) held*/
	unsigned int hlocks;		/*bitmap to indicate locks (32-49) held*/
	unsigned int dllocks;		/*bitmap to indicate locks(0-31) deleted*/
	unsigned int dhlocks;		/*bitmap to indicate locks (32-49) deleted*/
